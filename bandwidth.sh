...
function initial()
{
	opencdn="/home/data1/amengcdn"
#	opencdn="."
	opencdn_log="${opencdn}/logs"
	[ -d "$opencdn_log" ] || mkdir -p $opencdn_log

	opencdn_bw_log_dir="${opencdn}/stream"
	[ -d "$opencdn_bw_log_dir" ] || mkdir -p $opencdn_bw_log_dir

	opencdn_pipe="${opencdn}/pipe"
	[ -d "$opencdn_pipe" ] || mkdir -p $opencdn_pipe
