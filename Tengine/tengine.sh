...
mkdir -p /home/data1/nginx/logs/
mkdir -p /home/data1/nginx/run/
mkdir -p /home/data1/nginx/temp/
mkdir -p /home/data1/nginx/cache/
...
sed -i 's/\"nginx/\"AmengCDN Beta/i' /tmp/tengine-1.4.6/src/core/nginx.h
sed 's/>nginx</>AmengCDN Beta</' -i /tmp/tengine-1.4.6/src/http/ngx_http_special_response.c
sed -i 's/\" NGINX_VER \"/AmengCDN beta/' /tmp/ngx_cache_purge-2.1/ngx_cache_purge_module.c
